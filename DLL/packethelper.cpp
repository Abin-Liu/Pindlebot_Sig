...
BOOL StepToEntity(DWORD type, DWORD EntityID, WORD entityX, WORD entityY)
{
	...
	BYTE aPacket[17] = { 0 };
	aPacket[0] = 0x59;
	memcpy(aPacket + 1, &type, 4);
	memcpy(aPacket + 5, &EntityID, 4);
	memcpy(aPacket + 9, &entityX, 2);
	memcpy(aPacket + 13, &entityY, 2);
	server->GameSendPacketToServer(aPacket, 17); 
	aPacket[0] = 0x04;
	server->GameSendPacketToServer(aPacket,9); // step to entity
...
BOOL MoveCharTo(WORD x, WORD y)
{
	TakeStep(x, y);
	ForceClientViewWalk(x, y);
	return TRUE;
}

void MoveAway(WORD cx, WORD cy)
{
	WORD x = PLAYERX;
	WORD y = PLAYERY;
...
BOOL ForceClientViewWalk(WORD x, WORD y)
{
	...
	memcpy(p+2,&PLAYERID,4);               // Paste my ID
	memcpy(p+7,&x, 2); // New location x
	memcpy(p+9,&y, 2); // New location y
	memcpy(p+12,&PLAYERX,2);     // Old location X
	memcpy(p+14,&PLAYERY,2);     // Old location Y
...
{
	return ((abs(x - PLAYERX) <= offset) && (abs(y - PLAYERY) <= offset));
}

BOOL CastNoTarget(WORD SkillID, BOOL bRight)
{
	return CastOnMap(PLAYERX, PLAYERY, SkillID, bRight);
}
