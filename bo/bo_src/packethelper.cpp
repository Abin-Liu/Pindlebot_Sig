...
BOOL CastOnMap(DWORD dwPlayerID, WORD x, WORD y, WORD SkillID, BOOL bRight)
{
	...
	ForceClientCastOnMap(dwPlayerID, SkillID, x, y);
	return TRUE;
}
...
BOOL CastOnEntity(DWORD dwPlayerID, DWORD EntityID, WORD SkillID, BOOL bRight)
{
	...
	server->GameSendPacketToServer(aPacket, 9);
	ForceClientCastOnEntity(dwPlayerID, SkillID, EntityID);
	return TRUE;
}
...
BOOL MoveCharTo(DWORD dwPlayerID, WORD wPlayerX, WORD wPlayerY, WORD x, WORD y)
{
	TakeStep(x, y);
	ForceClientViewWalk(dwPlayerID, wPlayerX, wPlayerY, x, y);
	return TRUE;
}

void MoveAway(WORD wPlayerX, WORD wPlayerY, WORD cx, WORD cy)
{
	WORD x = wPlayerX;
	WORD y = wPlayerY;
...
void RandMove(WORD wPlayerX, WORD wPlayerY, WORD wRange)
{
	WORD x = (rand() % wRange) + 1;
	WORD y = (rand() % wRange) + 1;
	MoveAway(wPlayerX, wPlayerY, x, y);
}

BOOL ForceClientViewWalk(DWORD dwPlayerID, WORD wPlayerX, WORD wPlayerY, WORD x, WORD y)
{
	// Update client when running
	BYTE p[]={0x0f, 0, 0,0,0,0, 0x17, 0,0,0,0, 0, 0,0,0,0};
	memcpy(p+2,&dwPlayerID,4);               // Paste my ID
	memcpy(p+7,&x, 2); // New location x
	memcpy(p+9,&y, 2); // New location y
	memcpy(p+12,&wPlayerX,2);     // Old location X
	memcpy(p+14,&wPlayerY,2);     // Old location Y
	server->GameSendPacketToGame(p, 16);               // Move clientside
	return TRUE;
}

BOOL CheckPosition(WORD wPlayerX, WORD wPlayerY, WORD x, WORD y, WORD offset /*= 0*/)
{
	return ((abs(x - wPlayerX) <= offset) && (abs(y - wPlayerY) <= offset));
}
