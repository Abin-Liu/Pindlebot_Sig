...
#define D2S_TOMBOFIDENTIFY		0x00da
#define D2S_SCROLLOFIDENTIFY	0x00d9
#define D2S_TOMBOFTOWNPORTAL	0x00dc
...
// skill info flag bit fields definition
#define DSI_PASSIVE			(1 << 0) // Skill is passive
#define DSI_RIGHTONLY		(1 << 1) // Skill must be on right side, like paladin auras
#define DSI_TARGETABLE		(1 << 2) // Skill cannot target, like Nova
#define DSI_NEEDCORPSE		(1 << 3) // Skill requires a corpse, like Revive
#define DSI_ENABLEINTOWN	(1 << 4) // Skill is enabled in town
#define DSI_PHYSICAL		(1 << 5) // Skill causes physical damage
#define DSI_MAGICAL			(1 << 6) // Skill causes magical damage
#define DSI_FIRE			(1 << 7) // Skill causes fire damage
#define DSI_COLD			(1 << 8) // Skill causes cold damage
#define DSI_LIGHTNING		(1 << 9) // Skill causes lightning damage
#define DSI_POISON			(1 << 10) // Skill causes poison damage
#define DSI_KNOCKBACK		(1 << 11) // Skill knocks target(s) back
#define DSI_STUN			(1 << 12) // Skill stuns target(s)
#define DSI_AFFECTGROUP		(1 << 13) // Skill affects multuple targets
#define DSI_MELEE			(1 << 14) // Skill only affects target(s) within melee range
#define DSI_SUMMON			(1 << 15) // Skill summons minion(s), like Valkyrie

#define MAX_SKILLNAME_LEN	32 // Max length of a skill name

// D2SKILLINFO struct 
typedef struct _struct_d2skillinfo
{
	DWORD dwSkillInfoFlag; // Skill info flag
	char szSkillName[MAX_SKILLNAME_LEN]; // Skill name
	DWORD dwCoolDown; // Build-in delay in milliseconds
} D2SKILLINFO, *LPD2SKILLINFO;
...
public:
	...
	BOOL IsNameSame(LPCSTR lpName) const;
	CD2Skill();
	CD2Skill(WORD wSkillID);
	...
	BOOL NeedCorpse() const;
	BOOL IsTargetable() const;
	BOOL IsRightOnly() const;
	BOOL GetSkillName(LPSTR lpBuffer, DWORD dwMaxLen = MAX_SKILLNAME_LEN) const;
	WORD GetSkillID() const;

	void SetSkillID(WORD wSkillID);

	// Only thing we can actually do is to provide skill info exactly according
	// to what have been pre-defined by Blizzard Entertainment(c).
	static BOOL GetSkillInfo(WORD wSkillID, LPD2SKILLINFO lpDsi);
