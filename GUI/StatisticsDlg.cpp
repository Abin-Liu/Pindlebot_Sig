...
BOOL CStatisticsDlg::OnInitDialog() 
{
	...
	m_ini.SetPathName(g_sAppDir + _T("\\scripts\\output\\pindlebot.txt"));
	// read and display data from pindlebot.txt

	int nTotalGames = m_ini.GetProfileInt(_T("Log"), _T("Game"), 0);
	if (m_ini.GetProfileBool(_T("Log"), _T("Running"), FALSE))
		nTotalGames -= 1;

	DWORD dwRunTime = m_ini.GetProfileInt(_T("Log"), _T("Run Time"), 0);
